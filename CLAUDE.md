...
- **Deploy to OpenShift**: Run `make build-image && NAMESPACE=my-ns make deploy`. The deploy script requires `config/sankey-scorecard.yaml` to exist (override with `CONFIG_FILE=<path>`), creates a ConfigMap (`sankey-scorecard-config`) from it, deploys PostgreSQL (PVC, Deployment, Service), creates the `sankey-scorecard-db` secret (with generated password if not already present), and then deploys the app. Override DB credentials via `DB_USER`, `DB_PASSWORD`, `DB_NAME` env vars.
